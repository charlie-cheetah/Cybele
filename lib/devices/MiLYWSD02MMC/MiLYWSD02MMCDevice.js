...
MiLYWSD02MMCDevice.prototype.poll = async function() {
    let batInfo, timeInfo, sensorData;
    ...
    if (this.connected) {
        ...
        try {
            sensorData = MiLYWSD02MMCDevice.PARSE_DATA(
              await this.readCharacteristicAsync(this.characteristicsByUUID[MiLYWSD02MMCDevice.CHARACTERISTICS.sensorData])
            );
        } catch (e) {
            console.error(e);
            return this.queuePolling();
        ...
        if (batInfo && timeInfo && sensorData) {
...
MiLYWSD02MMCDevice.prototype.handleNotificationForDevice = async function (props) {
    console.log('handleNotificationForDevice')
