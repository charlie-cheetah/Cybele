...
const { Parser, SERVICE_DATA_UUID } = require("./parser");
const MiLYWSD02MMCMqttHandler = require("./MiLYWSD02MMCMqttHandler");
...
const MiLYWSD02MMCDevice = function MiLYWSD02MMCDevice(options) {
    ...
    this.mqttHandler = new MiLYWSD02MMCMqttHandler({
        device: this,
        prefix: "MiLYWSD02MMC"
    });
...
MiLYWSD02MMCDevice.prototype.handleMqttMessage = function(topic, message) {
...
MiLYWSD02MMCDevice.prototype.poll = async function() {
    let batInfo, timeInfo;
    console.log('MiLYWSD02MMCDevice connecting')

    this.tryConnectToClock(async(err) => {
        if (!err && this.connected) {
            ...
            if (batInfo && timeInfo) {
                ...
                console.info("Got invalid or missing data for " + this.friendlyName, batInfo, timeInfo, sensorData);
            }
        }
...
MiLYWSD02MMCDevice.prototype.handleAdvertisingForDevice = function (props) {
    ...
    if (props.ServiceData &&
      props.ServiceData.UUID &&
      props.ServiceData.UUID.toLowerCase() === SERVICE_DATA_UUID || false) {
        try {
            serviceData = new Parser(props.ServiceData.data, this.bindKey).parse();
            this.mqttClient.publish(MiLYWSD02MMCDevice.MQTT_PREFIX + this.id + "/state", JSON.stringify(serviceData.event));
        } catch (error) {
    ...
    /*if(props.Connected !== undefined) {
        console.log('props.Connected', props.Connected)
...
MiLYWSD02MMCDevice.prototype.tryConnectToClock = function (callback) {
    if (this.connected === false && this.connectionSemaphore.available()) {
        this.connectionSemaphore.take(() => {
            this.connectToClock(err => {
                if (err) {
...
MiLYWSD02MMCDevice.prototype.disconnectClock = async function() {
    if (this.connected && this.deviceInterface) {
        await this.deviceInterface.Disconnect(err => {
            if (!err) {
                this.connected = false
            }
        })
...
MiLYWSD02MMCDevice.prototype.setTime = function(time, callback) {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32LE(time, 0);

    this.semaphore.take(() => {
        this.characteristicsByUUID[MiLYWSD02MMCDevice.CHARACTERISTICS.time].WriteValue(buf, {mode: "request"}, err => {
...
MiLYWSD02MMCDevice.prototype.connectToClock = function (callback) {
    this.prepareDeviceInterface(async (err, deviceInterface) => {
        if(!err && deviceInterface) {
            ...
            try {
                await this.connectDeviceAsync(deviceInterface, 4000,2); //TODO: does this make sense?
            } catch(e) {
